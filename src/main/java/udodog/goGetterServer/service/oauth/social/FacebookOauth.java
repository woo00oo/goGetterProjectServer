...
public class FacebookOauth implements SocialOauth {
    ...
    @Value("${sns.facebook.url}")
    private String FACEBOOK_SNS_BASE_URL;

    @Value("${sns.facebook.client.id}")
    private String FACEBOOK_SNS_CLIENT_ID;

    @Value("${sns.facebook.callback.url}")
    private String FACEBOOK_SNS_CALLBACK_URL;

    @Value("${sns.facebook.client.secret}")
    private String FACEBOOK_SNS_CLIENT_SECRET;

    @Value("${sns.facebook.token.url}")
    private String FACEBOOK_SNS_TOKEN_BASE_URL;

    @Value("${sns.facebook.client.scope}")
    private String FACEBOOK_SNS_SCOPE;
    ...
    public String getOauthRedirectURL() {
        ...
        params.put("scope", FACEBOOK_SNS_SCOPE);
        params.put("response_type", "code");
        params.put("client_id", FACEBOOK_SNS_CLIENT_ID);
        params.put("redirect_uri", FACEBOOK_SNS_CALLBACK_URL);

        String parameterString = params.entrySet().stream()
                .map(x -> x.getKey() + "=" + x.getValue())
                .collect(Collectors.joining("&"));

        System.out.println(FACEBOOK_SNS_BASE_URL + "?" + parameterString);

        return FACEBOOK_SNS_BASE_URL + "?" + parameterString;
    }

    @Override
    public String requestAccessToken(String code) {
        String access_Token = "";
        String reqURL = FACEBOOK_SNS_TOKEN_BASE_URL;
        try {
            ...
            sb.append("&client_id=" + FACEBOOK_SNS_CLIENT_ID);
            sb.append("&client_secret=" + FACEBOOK_SNS_CLIENT_SECRET);
            sb.append("&redirect_uri=" + FACEBOOK_SNS_CALLBACK_URL);
            sb.append("&code=" + code);
            ...
            throw new IllegalArgumentException("알 수 없는 구글 로그인 Access Token 요청 URL 입니다 :: " + FACEBOOK_SNS_TOKEN_BASE_URL);
        }
        return access_Token;
    }
    ...
    public DefaultRes requestAccessTokenUsingURL(String access_token) {
        ...
        String reqURL = "https://graph.facebook.com/me?fields=id,email,name,picture&access_token=" + access_token;
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                String picture = element.getAsJsonObject().getAsJsonObject("picture").getAsJsonObject("data").get("url").getAsString();
            ...
            new IllegalArgumentException("알 수 없는 구글 로그인 Access Token 요청 URL 입니다 :: " + FACEBOOK_SNS_TOKEN_BASE_URL);
        }
